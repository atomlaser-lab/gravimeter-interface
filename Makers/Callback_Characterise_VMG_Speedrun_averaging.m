function Callback_Characterise_VMG_Speedrun_averaging(r)
...
if r.isInit()
    ...
    %     r.data.delta_key =
    %     randperm(r.data.delta_param*r.data.no_averages); %getting random keyr
...
elseif r.isSet()
    ...
    if r.c(1) < r.data.no_averages*r.data.delta_param+1
        r.make(r.devices.opt,'param1',r.data.delta(r.c(1)),'param2',r.data.time_for_delta,'param3',0,'params',1).upload;
        fprintf(1,'Run %d/%d, delta = %.3f MHz, time = %.1f microsec, phase = %.3f degrees\n',r.c.now,r.c.total,r.data.delta(r.c(1)),r.data.time_for_delta*1e6,0);
...
elseif r.isAnalyze()
    ...
    %     if (~img(1).raw.status.ok() || r.data.N(i1,1) == 0 || r.data.N(i1,1) > 1e7 || r.data.Rsum(i1,1) > 0.98 || r.data.Rsum(i1,2) > 0.98)
    if (~img(1).raw.status.ok() || r.data.N(i1,1) == 0 || r.data.N(i1,1) > 1e7)
        ...
        elseif ( r.data.N(i1,1) == 0 || r.data.N(i1,1) > 1e7)
    ...
    elseif ((r.c(1) > r.data.no_averages*r.data.delta_param) & (r.c(1) < r.data.no_averages*(r.data.delta_param + r.data.time_param) + 1))
        ...
        counter = i1 - r.data.counter;

        % plotting as you go
        A1=r.data.time_key';
        B1=r.data.Rsum([r.data.no_averages*r.data.delta_param:i1],2);
        [A1,index(1:counter)] = sortrows(A1(1:counter));
        B1 = B1(index(counter),:);
        A2=r.data.time_key';
        B2=r.data.Rsum([r.data.no_averages*r.data.delta_param:i1],1);
    ...
    elseif  r.c(1) > r.data.no_averages*(r.data.delta_param + r.data.time_param)
        ...
        errorbar(2*x,C1_mean - C2_mean,sqrt(C2_std.^2+C1_std.^2),"o","MarkerSize",5,"MarkerEdgeColor","blue","MarkerFaceColor",[0.65 0.85 0.90],'linewidth',2,'Color','blue')
        xlim(2*[min(x) max(x)])
        ylim([-1 1])
        xlabel('LG Rotated Phase (deg)')
        ylabel('N_1 - N_2')
        set(gcf,'color','white')
    end
