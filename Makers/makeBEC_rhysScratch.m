function sq = makeBEC_rhysScratch(varargin)
...
if LoadOpticalTrap_status == 1
    ...
    if OpticalEvaporation_status == 0
        sq.find('bias e/w').set(0);
        sq.find('bias n/s').set(0);
...
if OpticalEvaporation_status == 1
    ...
    sq.find('50W amp').after(t,sq.expramp(t,sq.find('50w amp').values(end),convert.dipole50(opt.params),0.5));
    sq.find('25W amp').after(t,sq.expramp(t,sq.find('25w amp').values(end),convert.dipole25(opt.params+0.25),0.5));
