...
if(VMG_Run)
    ...
    raman_delay_drop = 6e-3; %6e-3 %15e-3 %5e-3 %10.8e-3
    interrogation_time = 100e-6; %0.1e-3
    Traman_pi = 20e-6; %32e-6 %40e-6 %32e-6
    F2_tof = 10e-3; %raman_delay_drop + interrogation_time + Traman_pi + 10e-3 %raman_delay_drop + interrogation_time + Traman_pi + 2e-3 %10e-3
    ...
    AI_phi_LG = opt.param1; %opt.param1
    sq.anchor(timeAtDrop + raman_delay_drop);
    Power_raman_LG = 0.05;  %0.05 at 14GHz %carrier (swapped)
    Power_raman_G = 0.2; %0.2 at 14GHz %sideband (swapped)
    Delta_raman = 20.0263; %20.0583
...
%         % Triggering Phase Lock
%     sq.find('85 push').before(5e-3,0).after(15e-3,0);

%  if opt.params == 1
% % % 
%         %     single Raman pulse
%         sq.dds(2).set(110-Delta_raman/4,Power_raman_G,0);
%         sq.dds(1).set(110+Delta_raman/4,Power_raman_LG,0);
%         sq.delay(Traman_pi);
%         sq.dds(2).set(110,0,0);
%         sq.dds(1).set(110,0,0);
%         sq.delay(10e-6);
...
%  else
    ...
    %     BS1
    sq.dds(2).set(110-Delta_raman/4,Power_raman_G,0); %sideband
    sq.dds(1).set(110+Delta_raman/4,Power_raman_LG,0); %carrier
    sq.delay(Traman_pi/2);
    sq.dds(2).set(110-Delta_raman/4,0,AI_phi_G);
    sq.dds(1).set(110+Delta_raman/4,0,AI_phi_LG);
    %     sq.find('LG Shutter').set(1).after(100e-6,0); %trigger SLM

    sq.delay(interrogation_time);

    %     BS2
    sq.dds(2).set(110-Delta_raman/4,Power_raman_G,AI_phi_G); %sideband
    sq.dds(1).set(110+Delta_raman/4,Power_raman_LG,AI_phi_LG); %carrier
    sq.delay(Traman_pi/2);
...
if(SG_Pulse)
    ...
    SG_Pulse_time = 3.6e-3; %3.3e-3 %3.45 %2.45e-3 %2.9e-3 %8e-3
    SG_Amp = 70; %25
...
% makeImagingSequence(sq,'tof',opt.tof,'pulse time',20e-6,'repump delay',100e-6,...
...
makeImagingSequence_4_Images(sq,'tof',F2_tof,'tof2',F1_minus_F2_tof,'pulse time',20e-6,'repump delay',100e-6,...
    'repump time',200e-6,'cam time',5e-6,'cycle time',100e-3,'imaging freq',ImageFreq,'imaging amplitude',ImageAmp,...
    'fibre switch delay',1e-3,'imaging_field',imaging_field,'image type','horizontal');
