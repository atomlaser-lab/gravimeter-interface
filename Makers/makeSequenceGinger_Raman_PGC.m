...
if(CMOT)
    ...
    sq.find('3DMOT amp').set(TrapPtoV('trap',1)); % WAS 1.0 NOW 1.45
    ...
    if(PGC)
        ...
        %Trapping light
        sq.find('3DMOT freq').after(t,sq.minjerk(t,sq.find('3DMOT freq').values(end),FtoV('trap',62))); %79.8
        sq.find('3DMOT amp').after(t,sq.minjerk(t,sq.find('3DMOT amp').values(end),TrapPtoV('trap',0.75))); % WAS 0.8, NOW 0.85
        ...
        Tdepump = 1.5e-3;
        sq.find('3DMOT amp').set(TrapPtoV('trap',1)); % WAS 0.8, NOW 0.85
...
if(VMG_Run)
    ...
    raman_delay_drop = 1e-3; %5e-3 %10.8e-3
    interrogation_time = 1e-6; %1e-4 %6e-3
    Traman_pi = 10e-6; %24e-6 %14e-6 %18e-6 %16e-6
    F2_tof = raman_delay_drop + 1e-3; %7e-3 %20.1e-3 %11e-3
    ...
    Power_raman_LG = 1; %1 %carrier
    Delta_raman = opt.params; %20.64 %18.4 %20.005 %20.02 for LG1
    ...
    %     sq.find('CD0 Fast').before(raman_delay_drop,0.2); %7.52G
        sq.find('MOT bias').before(raman_delay_drop+1,1);
    %     %      sq.find('LG Shutter').before(raman_delay_drop+1,1).after(F2_tof+raman_delay_drop+1,0); %opening shutter
    %     % %                 sq.delay(1e-3);

    % % small field with the imaging coils
    %     sq.find('MOT bias').set(1);
    %     sq.find('MOT bias coil').before(20e-3,0);
    %     sq.find('Earth Bias 3').before(20e-3,opt.param2);
