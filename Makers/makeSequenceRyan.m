...
%
sq.find('2DMOT').set(1);
sq.find('3DMOT').set(0).after(opt.load_time - 3,1);
sq.find('87 push').set(1);
% 3D MOT beam settings
sq.find('3DMOT Freq').set(FtoV('trap',25.5));
sq.find('3DMOT amp').set(TrapPtoV('trap',0.9));
...
%Repump
sq.find('87 repump freq').set(FtoV('repump',10)); %-7
...
sq.find('CD0 Fast').set(dBtoV('normal',0));
sq.find('3DMOT freq').after(t,sq.minjerk(t,sq.find('3DMOT freq').values(end),FtoV('trap',79))); 
sq.find('3DMOT amp').after(t,sq.minjerk(t,sq.find('3DMOT amp').values(end),TrapPtoV('trap',0.85))); %0.5
...
sq.find('Raycus CW').after(t,sq.minjerk(t,0,DipolePtoV('raycus',8)));
sq.find('Redpower CW').after(t,sq.minjerk(t,0,DipolePtoV('RedPower',13)));

if opt.nd.enable_fb_laser
    sq.find('Feedback laser TTL').set(1);
    sq.find('feedback laser power').after(t,sq.linramp(t,0,DipolePtoV('feedback',opt.nd.fb_laser_power)));
end
% sq.find('feedback laser power').after(t,sq.minjerk(t,0,3));
...
sq.find('RedPower CW').after(t,sq.expramp(t,sq.find('RedPower CW').values(end),DipolePtoV('redpower',opt.redpower),TC));
sq.find('Raycus CW').after(t,sq.expramp(t,sq.find('Raycus CW').values(end),DipolePtoV('raycus',opt.raycus),TC));
sq.delay(Tevap);
% 
T = 200e-3;
t = linspace(0,T,51);
sq.find('Raycus CW').after(t,sq.linramp(t,sq.find('Raycus CW').values(end),DipolePtoV('raycus',opt.raycus + 0.1)));
sq.find('RedPower CW').after(t,sq.linramp(t,sq.find('RedPower CW').values(end),DipolePtoV('redpower',opt.redpower)));
sq.delay(T);

%% Non-destructive imaging
% sq.find('Raycus CW').after(20*opt.nd.cycle_time,DipolePtoV('raycus',final_dipole.FA + 0.2));
% if opt.params
%     num_mod_cycles = 10;
%     fmod = 40.9;
%     Tmod = 1/fmod;
...
%     sq.find('feedback laser power').after(t,DipolePtoV('feedback',opt.nd.fb_laser_power.*(1 + 0.25*sin(2*pi*fmod*t))));
%     sq.delay(num_mod_cycles*Tmod);
% end
% sq.find('feedback laser power').after(20e-3,DipolePtoV('feedback',opt.nd.fb_laser_power));

makeNDImagingSequence(sq,'pulse time',opt.nd.pulse_time,'cam time',opt.nd.pulse_delay,'cycle time',opt.nd.cycle_time,...
    'imaging freq',8.5,'imaging amplitude',opt.nd.pulse_power,'species',85,'num_images',opt.nd.num_images(1),...
    'pulse delay',opt.nd.pulse_delay);
% sq.delay(opt.params(1));
sq.delay(0.5);

% sq.delay(500e-3);
% if numel(opt.nd.num_images) > 1
%     makeNDImagingSequence(sq,'pulse time',opt.nd.pulse_time,'cam time',opt.nd.pulse_delay,'cycle time',opt.nd.cycle_time,...
%         'imaging freq',8.5,'imaging amplitude',opt.nd.pulse_power,'species',85,'num_images',opt.nd.num_images(2),...
%         'pulse delay',opt.nd.pulse_delay);
% end
