function varargout = makeSequence_Feedback_Jordan_Working_test(varargin)   
...
%%% Optimise:
optimiseAOM_VCO = 0; % this kicks the cloud at 40 ms into imaging to optimise the VCO bias. Set P_FBL = 0.5 for all stages before the optimise o/w P_FBL = 1.0. 
    ...
    if opt.nd.ref_images > 0       
        ...
        if FB_AOM == 1
            if optimiseAOM_VCO == 1
                sq.find('Feedback laser power').set(DipolePtoV('Feedback', 1.50)); % Set FB laser to 1.0 W 
            else
                sq.find('Feedback laser power').set(DipolePtoV('Feedback', 0.50)); % Set FB laser to 1.0 W 
            end 
        end 
...
if ODT_transfer == 1
    ...
    final_dipole_ramp.FA  = opt.raycus + opt.param1; % RAMP up
    final_dipole_ramp.RP  = opt.param2; % 0.3
    final_dipole_ramp.FBL = 1.5; 

    %%% Time vector
    TAdiabaticRamp = 200e-3; % 
    TDelay         =  50e-3;
    ...
    sq.find('Raycus CW'           ).after(t,sq.expramp(t,sq.find('Raycus CW'           ).values(end),DipolePtoV('raycus',  final_dipole.FA),0.5));
...
if ODT_ramp == 1
    ...
    final_dipole_ramp.RP  = opt.redpower + 0.0; % NORMAL USE WAS a +0.1 increase
...
if PowerModulate_ODT == 1
    ...
    nn = 4; % 10 if FBL o/w 4
    f_mod = sqrt(5/2) * 20.4; % opt.params;
    T_mod = 1/f_mod;
    ...
    if(opt.params)
        %%% Define the modulation to power:
        P_FBL = 0.5;
        dP_mod = 2.0; % 50% modulation depth
        final_dipole_mod.FBL = P_FBL * (1 + dP_mod * sin(2 * pi * f_mod * t));
...
% sq.anchor(sq.time - num_mod_cycles*Tmod/2);
    ...
    if optimiseAOM_VCO == 1
        P_FBL = 1.50;
        sq.find('feedback laser power').set(DipolePtoV('feedback',P_FBL)); % Max power
    else
        P_FBL = 1.25;
        sq.find('feedback laser power').set(DipolePtoV('feedback',P_FBL)); % NORMAL USE
    end 
