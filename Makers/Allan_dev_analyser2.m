...
%         title('Allan Deviation Data')
        ...
        contrast =  1; %0.0259*2; %0.07063
        DC_offset = 0*0.4156; %0.7318
        phase_offset = 0*3.426;
        l = 10;
        T = 1e-3;
...
%         cosphi = asin((N1-N2-DC_offset)/(2*contrast)) + phase_offset;
        ...
        if imag(cosphi(counter)) == 0
            phase(counter) = cosphi(counter);
            cosphi = cosphi(~isnan(cosphi));
% imag(cosphi(counter))
            if length(phase) > 2
