function Callback_MeasureVis_MeasureTemp_OAM_averaging(r)

if r.isInit()
    r.data.raycus = [5e-3:1e-3::100e-6:2e-3]; %for dipoles: [1.5:0.5:8]
    r.data.OAM = [0 500e-6:500e-6:2e-3];
    ...
    r.data.limit = 1;
    r.data.vis = NaN*ones(length(r.data.OAM),length(r.data.raycus));
    r.data.key = randperm(r.data.number_of_param*r.data.no_averages); %getting random key
    r.data.phase = r.data.phase(r.data.key);
    r.c.setup('var',r.data.phase,r.data.raycus,r.data.OAM);
elseif r.isSet()
    r.make(r.devices.opt,'param1',r.data.phase(r.c(1)),'param2',r.data.raycus(r.c(2)),'param3',r.data.OAM(r.c(3))).upload;
    fprintf(1,'Run %d/%d, Rotation = %.3f degrees, T = %.3f ms, TOF = %.3f\n',r.c.now,r.c.total,2*r.data.phase(r.c(1)),1e3*r.data.raycus(r.c(2)),1e3*r.data.OAM(r.c(3)));
elseif r.isAnalyze()
    i1 = r.c(1);
    i2 = r.c(2);
    i3 = r.c(3);
...
%     xlim([0 360])
    ...
    if r.c(1) == length(r.data.phase)
        ...
        saveas(gcf,sprintf('D:\\data\\VMG_autosave\\TOF = %.3f.png',r.data.raycus(i2),1e3*r.data.OAM(i3)));
        r.data.offset(i3,i2) = f.b;

        figure(43);clf;
        plot(r.data.raycus,r.data.offset,'o')
        ylim([0 max(max(r.data.vis))])
        grid on;
        enhformat('Interrogation Time (s)','Offset')
    end
