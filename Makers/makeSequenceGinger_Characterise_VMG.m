function varargout = makeSequenceGinger_Characterise_VMG(varargin)
...
if(VMG_Run)
    ...
    raman_delay_drop = 6e-3; %5e-3 %10.8e-3
    interrogation_time = 1e-6; %6e-3
    Traman_pi = opt.param2; %18e-6
    F2_tof = 13.04e-3; %raman_delay_drop + interrogation_time + Traman_pi + 2e-3
    F1_tof = F2_tof + 4.1e-3; %3e-3 minimum with smallest frame for BEC
    F1_minus_F2_tof = F1_tof - F2_tof; %needs to be 12ms minimum for full frame
    AI_phi_G = 0; %abs(180 - opt.params*180*1e3)
    AI_phi_LG = opt.param3;
    sq.anchor(timeAtDrop + raman_delay_drop);
    Power_raman_G = 1;  %0.3 %sideband
    Power_raman_LG = 1; %0.6 %1 %carrier
    Delta_raman = opt.param1; %20.0333 %20.02 + 1.967155096615827e-2
    ...
    sq.find('85 push').before(10e-3,0).after(107e-3 + interrogation_time + Traman_pi,0);
...
makeImagingSequence_4_Images(sq,'tof',F2_tof,'tof2',F1_minus_F2_tof,'pulse time',25e-6,'repump delay',100e-6,...
