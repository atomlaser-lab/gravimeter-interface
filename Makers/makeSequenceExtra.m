...
ImageAmp = 5; %5 until and including mag load, 7 for high OD samples on horizontal imaging
...
sq.find('3DMOT amp').after(t,sq.minjerk(t,sq.find('3DMOT amp').values(end),TrapPtoV('trap',1.3))); %1.3
...
sq.find('RedPower CW').after(t,sq.expramp(t,sq.find('RedPower CW').values(end),DipolePtoV('redpower',final_dipole.RP),0.6)); %0.48 %0.42
...
if(0)
pulse_freq = 38.263; %this value was quoted to work on 16/9/22
...
raman_delay_drop = 3e-3; %35e-3
% raman_delay_drop = opt.params;
% interrogation_time = 10e-3; %1.2e-3
% interrogation_time = opt.params;
Traman_pi = 100e-6; % 30e-6 for charge 1, 20e-6 for charge 2, 35e-6 for charge 3 
% Traman_pi = opt.params;
F2_tof = opt.params;
% F2_tof = raman_delay_drop + Traman_pi + 1e-3;
% F2_tof = raman_delay_drop + Traman_pi/2 + interrogation_time + Traman_pi/2 + 10e-6;
F1_tof = F2_tof + 12e-3; %6e-3 with shorter frame
F1_minus_F2_tof = F1_tof - F2_tof; %needs to be 12ms minimum for full frame
% AI_phi = opt.params;
AI_phi = 0;
sq.anchor(timeAtDrop + raman_delay_drop);
Power_raman_G = 1; %sideband
Power_raman_LG = 1; %carrier 
...
sq.delay(Traman_pi); %Traman_pi/2
sq.dds(1).set(110,0,0);
sq.dds(2).set(110,0,0);
% sq.delay(10e-6);
