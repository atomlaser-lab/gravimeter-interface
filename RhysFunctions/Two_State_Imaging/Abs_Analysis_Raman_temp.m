function img = Abs_Analysis_Raman_temp(varargin)
%% Inputs
% % % Plotting Options
FigNum = 1;
dispOD = [0,1];
...
% % % ROI of single cloud
roiCol{1} = [LeftColumn RightColumn];
roiRow{1} = [TopRow BottomRow];

roiCol{2} = [LeftColumn2 RightColumn2];
roiRow{2} = [TopRow2 BottomRow2];
%% Imaging parameters
...
img{1} = AbsorptionImage.empty;
img{2} = AbsorptionImage.empty;
...
%         img(nn,2) = AbsorptionImage(BinaryImageData);
        img{1}(nn,1) = AbsorptionImage(BinaryImageData);
        img{2}(nn,1) = AbsorptionImage(BinaryImageData);
    end
end
...
for kk = 1:2
    for jj = 1:numImages
        ...
        %
        img{kk}(jj).constants.copy(imgconsts{kk});
        img{kk}(jj).raw.copy(raw(jj));
        if kk == 1
            img{kk}(jj).raw.images = img{kk}(jj).raw.images(:,:,[1,3,4]); % atoms in F = 2 manifold
        else
            img{kk}(jj).raw.images = img{kk}(jj).raw.images(:,:,[2,3,4]); % atoms in F = 1 manifold
        end
        img{kk}(jj).setClouds(size(roiRow{kk},1));
        for nn = 1:numel(img{kk}(jj).clouds)
            img{kk}(jj).clouds(nn).fitdata.set('roirow',roiRow{kk}(nn,:),'roiCol',roiCol{kk}(nn,:),...
        ...
        %
        img{kk}(jj).makeImage([1,2,3]);

        if useFilt
            img{kk}(jj).butterworth2D(filtWidth);
        end
        %
        % Fit clouds
        %
        ...
        if plotOpt
            if numImages == 1
                ...
                img{kk}(jj).plotAllData(dispOD,plotROI);
                h = gcf;
                ax = h.Children(end);
        ...
        %% Print summaries
        [labelStr,numStr] = img{kk}(jj).labelOneROI;
        if jj == 1
            disp(labelStr);
        end
        disp(numStr);
