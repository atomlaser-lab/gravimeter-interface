function sq = makeBEC_rhys_tempBackup(varargin)
...
% opt.misc.detuning2 = opt.detuning;

imageVoltage = convert.imaging(opt.misc.detuning2 - 1.8); % detuning for F = 2 atoms
imageVoltage2 = convert.imaging(opt.detuning - 1.8); % detuning for F = 1 atoms
...
if opt.CMOT_status == 1
    ...
    sq.find('3D MOT freq').set(convert.mot_freq(-26));
    ...
    sq.find('3D coils').set(convert.mot_coil(1.45)); %1.45
    sq.find('bias e/w').set(0);
    sq.find('bias n/s').set(0);
...
if opt.PGC_status == 1
    ...
    % Detune/reduce light
    sq.find('3D MOT Amp').after(t,f(4.5,3.9));
    sq.find('3D MOT Freq').after(t,f(sq.find('3D MOT Freq').values(end),convert.mot_freq(-71)));
...
if opt.LoadMagTrap_status == 1
    ...
    f = @(vi,vf) sq.linramp(t,vi,vf);
    sq.find('MOT coil ttl').set(1);    
    sq.find('3D coils').after(t,f(convert.mot_coil(2),convert.mot_coil(6)));
...
if opt.OpticalEvaporation_status == 1
    ...
    sq.find('50W amp').after(t,sq.expramp(t,sq.find('50w amp').values(end),convert.dipole50(5),0.5)); %4.8
    sq.find('25W amp').after(t,sq.expramp(t,sq.find('25w amp').values(end),convert.dipole25(5+0.25),0.5));
...
sq.find('3D mot amp ttl').set(0);
sq.find('bias e/w').before(5e-3,0); %200 ms before
sq.find('bias n/s').before(5e-3,0);
sq.find('bias u/d').before(5e-3,0);
...
%     I_factor = MakeIntensityRamp('amp',0,'width',PulseWidth,'dt',dt,'NumPulseWidths',2);
    I_factor = 1;

    MakePulseSequence_Rhys(sq.dds,'k',k,'t0',tvs,'T',T_int,'width',PulseWidth,'dt',dt,...
        'Tasym',Tasym,'phase',[0,0,0],'start_order',0,'order',1,'chirp',chirp,'delta',opt.params,...
        'power',0.03*[1,0,0],'PulseType','Square','NumPulseWidths',2,'I_factor',I_factor,'I_ratio',1);

% % % %     Test to see DDS is turning on
%     sq.dds(1).set(110,1,0)
%     sq.dds(2).set(110,1,0)
%     sq.dds(1).after(1,110,1,0)
%     sq.dds(2).after(1,110,1,0)
%     sq.dds(1).after(2,110,0,0)
%     sq.dds(2).after(2,110,0,0)    
...
if strcmpi(Abs_Analysis_parameters.camera,'in-trap') || strcmpi(Abs_Analysis_parameters.camera,'drop 2')
    if RamanImagingSequence == 1
        ...
        makeImagingSequence(sq,'type',Abs_Analysis_parameters.camera,'tof',opt.tof,...
            ...
            'manifold',1,'includeDarkImage',true,'cycle time',150e-3,'manifold',Manifold);
    end
