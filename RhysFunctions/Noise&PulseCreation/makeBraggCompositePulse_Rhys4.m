function makeBraggCompositePulse_Rhys4(dds,varargin)
...
if strcmpi(PulseShape,'Gaussian') == 0 && strcmpi(PulseShape,'Square') == 0
    warning('Pulse Shape must be Gaussian or Square')
    return
end
...
if strcmpi(PulseShape,'Gaussian') == 1
    ...
    for ii = 1:numSegments
        if ii == 1
            tPulseSegment = (-t_segments(ii):dt:t_segments(ii))';
            SegmentCentres(ii) = t0;
        else
            tPulseSegment = (-NumStd*t_segments(ii):dt:NumStd*t_segments(ii))';
            tPulseSegment = tPulseSegment - tPulseSegment(1) + t(end);
        ...
        t_RisingEdge(ii) = SegmentCentres(ii) - t_segments(ii)/2;
        t_FallingEdge(ii) = SegmentCentres(ii) + t_segments(ii)/2;
...
%         if numSegments == 1
            ...
            P(:,1) = P(:,1) + I_Noise_factor.'.*power1;
            P(:,2) = P(:,2) + I_Noise_factor.'.*power2;

            freq(idx,1) = DDSChannel.DEFAULT_FREQ + mirrorSwitch*0.25/1e6*(chirp*tc + (2*start_order+order)*4*recoil) + (FreqError*recoil)/2e6;
            freq(idx,2) = DDSChannel.DEFAULT_FREQ - mirrorSwitch*0.25/1e6*(chirp*tc + (2*start_order+order)*4*recoil);
%         else
%             PulseStart = find(abs(t - (t_RisingEdge(nn))) < dt/2,1,'last');
%             PulseEnd = find(abs(t - (t_FallingEdge(nn))) < dt/2,1,'first');
%             SquareShape = zeros(1,length(t));
%             SquareShape(PulseStart:PulseEnd) = 1;
%             idx = SquareShape == 1;
%             P(:,2) = P(:,2) + (power2.*I_Noise_factor.*SquareShape).';
%             P(:,1) = P(:,1) + (power1.*I_Noise_factor.*SquareShape).';
