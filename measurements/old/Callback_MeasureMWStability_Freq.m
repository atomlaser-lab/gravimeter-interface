function Callback_MeasureMWStability_Freq(r)

if r.isInit()
    
    r.data.df = const.randomize(2:0.5:10); %in kHz %broad scan
%          r.data.df = const.randomize(5.5:0.3:7); %in kHz %small scan
% 
    r.data.freq1 = const.f_Rb_groundHFS/1e6 - 315e-3 + r.data.df*1e-3;
%     r.data.freq1 = const.f_Rb_groundHFS/1e6 - 315e-3  + 2.109e-3*ones(size(r.data.df));
%     r.data.freq2 = const.f_Rb_groundHFS/1e6 + r.data.df*1e-3;
    r.data.freq2 = const.f_Rb_groundHFS/1e6*ones(size(r.data.df));
    
    r.c.setup('var',r.data.df,1:100);
elseif r.isSet()
    
    r.make(r.devices.opt,'tof',217.5e-3).upload;
    %
    % These commands are for list-mode operation
    %
%     r.devices.rs.writeList([r.data.freq1(r.c(1))/2,r.data.freq2/2],[6,6]);
%     r.devices.rs.writeState('on');
    r.devices.mku.writeList(r.data.freq1(r.c(1))/2*1e6,r.data.freq2(r.c(1))/2*1e6);
    fprintf(1,'Run %d/%d, F = %.6f kHz\n',r.c.now,r.c.total,...
        r.data.df(r.c(1)));
    
elseif r.isAnalyze()
    i1 = r.c(1);
    i2 = r.c(2);
    pause(0.1);
    
    
    img = Abs_Analysis_old('last');
    if ~img.raw.status.ok()
        %
        % Checks for an error in loading the files (caused by a missed
        % image) and reruns the last sequence
        %
        r.c.decrement;
        return;
    end
    
%     Store raw data
    
    r.data.files{i1,i2} = img.raw.files;
    
%     Get processed data
    
    r.data.N(i1,:,i2) = img.get('N');
    r.data.Nsum(i1,:,i2) = img.get('Nsum');
    r.data.R(i1,:,i2) = r.data.N(i1,:,i2)./sum(r.data.N(i1,:,i2));
    r.data.Rsum(i1,:,i2) = r.data.Nsum(i1,:,i2)./sum(r.data.Nsum(i1,:,i2));
      
    figure(98);clf;
    subplot(1,2,1)
    plot(r.data.df(1:i1),r.data.R(1:i1,:,i2),'o');
    plot_format('Freq [kHz]','Population','',12);
% %     h = legend('m = -1','m = 0');
%     set(h,'Location','West');
    ylim([0,1]);
    title(' Microwave frequency using fit over OD')
    grid on
    hold on;
    
%     subplot(1,3,2)
%     plot(r.data.df(1:i1),r.data.R(1:i1,:),'sq');
%     hold off;
%     plot_format('Freq [MHz]','Population','',12);
%     h = legend('m = -1','m = 0');
%     set(h,'Location','West');
%     title(' Raman frequency using ROI')
%     grid on;
%     if r.c.done
%     tNow = datestr(now);
%         caption = sprintf('Determination of Raman frequency %s', tNow);
%         sgtitle(caption)
%     end
    
    subplot(1,2,2)
    plot(r.data.df(1:i1),r.data.N(1:i1,:,i2),'o');
    plot_format('Freq [kHz]','Number','',12);
    h = legend('m = -1','m = 0');
    set(h,'Location','West');
    grid on
    hold on;
    
    if r.c.done(1)
        nlf = nonlinfit(r.data.df*1e-3,r.data.R(:,1,i2),1e-2);
        nlf.setFitFunc(@(A,R,x0,x) A*(1 - 4*R.^2./(4*R^2+(x-x0).^2).*sin(2*pi*sqrt(4*R^2+(x-x0).^2).*372/2).^2));
        [~,idx] = min(nlf.y);
        nlf.bounds2('A',[0.9,1,0.95],'R',[0,10,0.8]*1e-3,'x0',[min(nlf.x),max(nlf.x),nlf.x(idx)]);
        nlf.fit;
        r.data.rabi(i2,1) = nlf.c(2,1)*1e3;
        r.data.f0(i2,1) = nlf.c(3,1)*1e3;
        fprintf(1,'Rabi frequency: %.3f kHz, Center = %.3f kHz\n',nlf.c(2,1)*1e3,nlf.c(3,1)*1e3);
        subplot(1,2,1);
        hold on
        xplot = linspace(min(nlf.x),max(nlf.x),1e2);
        plot(xplot*1e3,nlf.f(xplot),'-');
        hold off;
        
        figure(190);clf;
        plot(1:i2,r.data.f0,'o-');
        plot_format('Run','Center freq [kHz]','',10);
        
    end
end