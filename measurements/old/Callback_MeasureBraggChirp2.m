function Callback_MeasureBraggChirp2(r)

if r.isInit()
    
    r.data.T = [2e-3,4e-3,6e-3];
    tmp = 5e3:5e3:100e3;
    tmp = [tmp;-tmp];
    r.data.chirp = [0;tmp(:)];
    r.c.setup('var',r.data.T,r.data.chirp);
elseif r.isSet()
    
    r.make(r.devices.opt,'bragg',{'chirp',r.devices.opt.bragg.chirp + r.data.chirp(r.c(2)),'T',r.data.T(r.c(1))});
    r.upload;
    fprintf(1,'Run %d/%d, Chirp = %.3f MHz/s, T = %.3f ms\n',r.c.now,r.c.total,...
        r.data.chirp(r.c(2))/1e6,r.data.T(r.c(1))*1e3);
...
%     subplot(1,2,1);
    cla;
    h = plot(r.data.chirp(1:i2)/1e3,r.data.R(:,1:i2),'o');
    for nn = 1:numel(h)
