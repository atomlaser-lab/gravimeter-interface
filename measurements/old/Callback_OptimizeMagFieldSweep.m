function Callback_OptimizeMagFieldSweep(r)

if r.isInit()
    
    r.data.v = const.randomize(9.4:0.025:9.85);
    
    r.c.setup('var',r.data.v);
elseif r.isSet()
    
    r.make(r.devices.opt,'tof',217.5e-3,'params',[r.data.v(r.c(1)),0.05]).upload;
    %
...
%     r.data.d{i1,1} = dout;
    ...
    figure(99);clf;
    subplot(1,2,1)
    plot(r.data.v(1:i1),r.data.R(1:i1,:),'o');
    plot_format('Voltage [V]','Population','',12);
% %     h = legend('m = -1','m = 0');
%     set(h,'Location','West');
    ylim([0,1]);
...
%     end
    ...
    subplot(1,2,2)
    plot(r.data.v(1:i1),r.data.N(1:i1,:),'o');
    plot_format('Voltage [V]','Number','',12);
    h = legend('m = -1','m = 0');
    set(h,'Location','West');
    grid on
    hold on;
